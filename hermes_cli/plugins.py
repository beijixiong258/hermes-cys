...
class PluginManager:
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        for manifest in winners.values():
            ...
            # Home Assistant and Yuanbao are intentionally dead platforms in
            # this build. Do not even register their deferred platform loader;
            # otherwise a later registry lookup could import and initialize
            # the old adapter despite the UI/configuration removal.
            if manifest.kind == "platform" and self._platform_name_from_manifest(manifest) in {
                "homeassistant",
                "yuanbao",
            }:
                logger.debug("Skipping disabled platform plugin: %s", lookup_key)
                continue
