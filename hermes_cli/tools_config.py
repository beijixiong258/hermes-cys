...
def _xai_credentials_present() -> bool:
    ...
    Used by provider readiness checks for xAI-backed capabilities. Does not
    hit the network; it only inspects the local auth store and environment.
    """
    try:
...
def _get_effective_configurable_toolsets():
    ...
    already appears in ``CONFIGURABLE_TOOLSETS`` is skipped, so the
    built-in label/description wins.
...
TOOL_CATEGORIES = {
    ...
    "image_gen": {
        ...
        #     distinct setup UX.
        # Mirrors the shape other plugin-managed categories use today.
        "providers": [
            {
                "name": "Nous Subscription",
                "badge": "subscription",
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict, *, use_gateway: bool = False) -> None:
    ...
    A provider picked through the Nous-managed flow must keep routing through
