...
# config.yaml section their `model` key lives in.
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    Plugin-backed rows carry ``image_gen_plugin_name``; the managed
    "Nous Subscription" image row instead carries the legacy
