...
def test_get_platform_tools_homeassistant_is_removed_even_when_token_set(monkeypatch):
    """The removed Home Assistant toolset cannot be revived by HASS_TOKEN."""
    ...
    assert "hermes-homeassistant" not in cron_enabled
    assert "moa" not in cron_enabled

    cli_enabled = _get_platform_tools({}, "cli")
    assert "homeassistant" not in cli_enabled
    assert "hermes-homeassistant" not in cli_enabled


def test_get_platform_tools_homeassistant_is_removed_even_from_active_profile(monkeypatch):
    """Profile-scoped HASS_TOKEN cannot restore the removed toolset."""
    ...
    try:
        ...
        assert "hermes-homeassistant" not in _get_platform_tools({}, "cron")
        assert "homeassistant" not in _get_platform_tools({}, "cli")
        assert "hermes-homeassistant" not in _get_platform_tools({}, "cli")
    finally:
