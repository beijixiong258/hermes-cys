...
class TestYuanbaoConfig:
    ...
    def test_get_connected_platforms_excludes_removed_yuanbao(self):
        # The removed platform stays out of the connected list even when its
        # historical credentials are present.
        ...
        # key + secret both present → still excluded
        ...
        assert Platform.YUANBAO not in platforms2
...
class TestGatewayRunnerRegistration:
    ...
    def test_runner_does_not_create_yuanbao_adapter(self):
        """Removed Yuanbao cannot be created by the Gateway runner."""
...
class TestToolset:
    def test_yuanbao_toolset_registered(self):
        """The removed Yuanbao toolset is absent from the runtime toolsets."""
        import importlib
        ...
        assert "hermes-yuanbao" not in toolsets_dict
